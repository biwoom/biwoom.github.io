...
0단계: 현재 Entity 기반 점검
1단계: /net 정적 진입 페이지
2단계: /net/explore 이름카드 검색결과·탐색 페이지
...
히어로 안에 이름카드 전용 검색폼 UI 배치
히어로 아래 최신 이름카드 리스트 출력
...
## 4. 2단계: /net/explore 이름카드 검색결과·탐색 페이지

### 목표

`/net` 진입 메인페이지와 별도로, NET의 검색결과·탐색 화면 기본 골격을 만든다.

`/net`은 히어로, 이름카드 전용 검색폼, 최근 이름카드만 담는 진입 페이지로 유지한다.
...
태그 prefix가 실제 이름카드 분류에 도움이 되는가?
...
사용자가 “NET은 이름을 탐색하는 도구”라는 느낌을 받을 수 있으면 4단계로 간다.
...
이름카드의 이름, 별칭, 원어, 설명, 태그를 대상으로 간단한 클라이언트 검색을 구현한다.

### 구현 내용

```txt
검색 input 활성화
이름카드에 검색용 data-search 속성 추가
검색어 입력 시 카드 목록 필터링
...
카드를 클릭했을 때 우측 패널에 이름카드 요약 정보를 보여준다.

### 구현 내용

```txt
이름카드 클릭 이벤트 추가
우측 패널에 선택 이름카드 정보 출력
...
선택 이름카드 주변의 1차 관계만 보여주는 미니 관계망 그래프를 구현한다.
...
## 11. 9단계: 장소 이름카드 지도 탭 placeholder
...
현재는 장소 이름카드의 위치 정보와 관련 문서를 먼저 제공합니다.
```
...
사용자가 완성한 고대 인도 SVG 지도를 장소 이름카드와 연결한다.
...
장소 이름카드의 map.markerId 또는 관련 필드가 준비되어 있어야 한다.
```
...
장소 이름카드의 placeId와 SVG marker id 연결
장소 선택 시 marker 강조
지도 탭에서 SVG 출력
```
...
장소 이름카드와 지도 marker 연결이 유지보수 가능한가?
```
...
2. 기존 BaseLayout 사용
3. NET 히어로와 이름카드 전용 검색폼 UI 작성
4. getCollection('entities')로 최신 이름카드 6~12개 출력
